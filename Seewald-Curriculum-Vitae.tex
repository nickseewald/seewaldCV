% !TeX TS-program = xelatex

\documentclass[letterpaper,11pt]{article} % Default font size and paper size

\usepackage[margin=.95in]{geometry}
\usepackage{datetime}
\usepackage{fontspec}
\usepackage[T1]{fontenc}
% \usepackage[utf8]{inputenc}
% \usepackage[american]{babel}
\usepackage{csquotes}
\newcommand*\publistbasestyle{ieee}

\usepackage[%
backend=biber,
casechanger=auto,
maxbibnames=99,
giveninits=true,
terseinits=true,
isbn=false,
block=space,
style=publist,
fixyear=false,
defernumbers=true,
refsegment=section,
hlyear=false,
plauthorhandling=highlight,
% plauthorname={Seewald},
annotations={student},
nameorder=family-given,
plnumbering=local-descending
]{biblatex}
\plauthorname{Seewald}
\addbibresource[label=peerreviewed]{articles.bib}
\addbibresource[label=preprints]{preprints.bib}
\addbibresource[label=chapters]{chapters.bib}
\addbibresource[label=abstracts]{abstracts.bib}
\addbibresource[label=underreview]{underReview.bib}
\addbibresource[label=altmedia]{altMedia.bib}
\ExecutePublistOptions{plsorting=ddt}
% \nocite{*}

% biblatex URL line breaking
\setcounter{biburllcpenalty}{7000}

\renewbibmacro{in:}{}

\DeclareFieldFormat{labelnumberwidth}{#1.}

\AtEveryBibitem{%
	\clearfield{urlyear}
	\clearfield{urlmonth}
	\clearfield{url}
	\clearfield{month}
	\clearfield{day}
}

\AtDataInput{%
	\csnumgdef{entrycount:\therefsection}{%
		\csuse{entrycount:\therefsection}+1}}

% \DeclareFieldFormat{labelnumber}{\mkbibdesc{#1}}    
% \newrobustcmd*{\mkbibdesc}[1]{%
	%   \number\numexpr\csuse{entrycount:\therefsection}+1-#1\relax}

\DeclareNameFormat{given-family}{%
	\ifgiveninits%
	{\ifthenelse{\equal{\namepartfamily}{Seewald}}%
		{\textbf{\namepartfamily\addspace\namepartgiveni\namepartsuffix}}%
		{\ifthenelse{\equal{\namepartfamily}{Moin} \OR \equal{\namepartfamily}{Taranto} \OR \equal{\namepartfamily}{Mayer}}%
			{\underline{\namepartfamily\addspace\namepartgiveni\namepartsuffix}}%
			{\namepartfamily\addspace\namepartgiveni\namepartsuffix}}%
		\ifthenelse{\value{listcount} < \value{liststop}}%
		{\addcomma\addspace}%
		{\ifthenelse{\ifmorenames}{~et \,al \adddot}}%
	}{}%
}

% No commas between last name and first initials
\renewcommand*{\revsdnamepunct}{}
% Annotation before last name
\renewcommand*{\mkbibnamefamily}[1]{\bplannot #1}

%\newfontfamily\headingfont{DM_Serif_Display.ttf}
%\newfontfamily\headingfont{Source Serif Pro}
\newfontfamily\headingfont{IBM Plex Serif}
\setmainfont{IBM Plex Sans}
\setmonofont{IBM Plex Mono}[Scale=MatchLowercase]
\setsansfont{IBM Plex Sans}
%\setmainfont{HindGuntur-Regular.ttf}[BoldFont= HindGuntur-Bold.ttf]

\usepackage{fontawesome5}
%\newcommand*\faMobile{{\FA\symbol{"F10B}}}

\usepackage{enumerate}
\usepackage{etaremune}
\usepackage[inline]{enumitem}
\usepackage{titling}
\usepackage{titlesec}
\usepackage{float}
% \usepackage[toc,enum,bib,lineno,eqno]{tabfigures}
\usepackage[dvipsnames]{xcolor} % Required for specifying custom colors
% \usepackage{ltablex} % longtable for tabularx?
\usepackage{tabularx}
% \usepackage{multirow}
\usepackage{hyphenat}
\usepackage{calc}

\usepackage{hyperref} % Required for adding links and customizing them
\definecolor{linkcolour}{rgb}{0,0.2,0.6} % Link color
\hypersetup{colorlinks,breaklinks,urlcolor=linkcolour,linkcolor=linkcolour} % Set link colors throughout the document
\urlstyle{tt}

% For enumitem description
\newcommand{\appendColon}[1]{#1:}
\setlist[description]{%
	font=\it\appendColon,
}
% Conference list definition
\newlist{conflist}{itemize}{1}
\setlist[conflist, 1]{%
	topsep=0pt,
	labelwidth=0pt,
	leftmargin=1em,
	label=-
}
% Inline list indent
% \setlength{}{}

% ----- Count table rows -----
\makeatletter

% Define counters
\newcounter{currentCountdown}   % Will hold the total read from the file
\newcounter{rowTracker}    % Counts the rows as we build the table

% Write table identification macro
\newcommand{\initRevTable}[1]{%
	\gdef\CurrentTableID{#1}
	\setcounter{rowTracker}{0} %reset row tracker for current table
	% check if count already exists for current table
	\@ifundefined{TableCount#1}{%
		\setcounter{currentCountdown}{0} %first run
	}{%
		%if found, set countdown to the saved value
		\setcounter{currentCountdown}{\csname TableCount#1\endcsname}
	}%
}

% 3. Define the row command
\newcommand{\autoRevNum}{%
	\ifx % Safeguard: Ignore tabularx width-calculation trials
	% A. Print the number
	\ifnum\value{currentCountdown}=0%
	\textbf{??}% Placeholder for the very first run
	\else
	\thecurrentCountdown.% Print the countdown
	\addtocounter{currentCountdown}{-1}% Decrement
	\fi
}

% 4. Define a command to save the total to the .aux file
\AtEndEnvironment{tabularx}{%
	\@ifundefined{CurrentTableID}{%
	}{%
	\immediate\write\@auxout{%
		\string\expandafter%
		\string\gdef\string\csname\space% 
		TableCount\CurrentTableID%
		\string\endcsname{\therowTracker}}}}

\newcommand\rowcount{\refstepcounter{rowTracker}}

\makeatother


% ----- TITLE -----
\titleformat{\section}{\Large\raggedright\headingfont}{}{0em}{}[\titlerule] % Text formatting of sections
% \titleformat{\subsection}{\large\raggedright\headingfont}{\thesection}{1em}{\hrule} % Text formatting of sections

\titleformat*{\subsection}{\large\headingfont}
\titleformat*{\subsubsection}{\headingfont}
\renewcommand{\maketitlehooka}{\headingfont}

\newdateformat{cvdate}{\THEMONTH-\THEYEAR}

\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\fancyfoot[L]{\footnotesize{N.J. Seewald}}
\fancyfoot[C]{\footnotesize{\thepage}}
\fancyfoot[R]{\footnotesize{\monthname \ \the\year}}

\fancypagestyle{firstpage}{%
	\fancyhf{}
	\fancyfoot[C]{\footnotesize \thepage}
	\fancyfoot[R]{\footnotesize{\monthname \ \the\year}}
	\renewcommand{\headrulewidth}{0pt} % removes horizontal header line
}

\begin{document} 
	
	\begin{center}
		\headingfont
		\huge{\textbf{Nicholas J. Seewald, Ph.D.}} \\
		%\huge{\textbf{NICHOLAS J. SEEWALD, Ph.D.}} \\
		\Large{Curriculum Vitae}
	\end{center}
	
	\begin{center}
		\faIcon[regular]{envelope} \  \href{mailto:nicholas.seewald@pennmedicine.upenn.edu}{\texttt{seewaldn@pennmedicine.upenn.edu}} \quad
		\faLaptop \  \href{https://www.nickseewald.com}{\texttt{nickseewald.com}} \quad
		\faGithub  \  \href{https://www.github.com/nickseewald}{\texttt{nickseewald}}
	\end{center}
	
	\thispagestyle{firstpage} % suppress footer on this page
	\hyphenation{post-meno-pausal} % fix weird hyphenation in paper title
    \hyphenation{chemo-therapy}
    \hyphenation{multi-ple-assign-ment}
	
	% CONTACT ----------------------------------------------
	
	\section*{Professional Data}
	
	\begin{minipage}[t]{.61\textwidth}
		Department of Biostatistics, Epidemiology, and Informatics \\
		University of Pennsylvania Perelman School of Medicine \\
		3600 Civic Center Drive \\
		Room 3W-103 \\
		Philadelphia, PA 19104
	\end{minipage}
	
	% EDUCATION ------------------------------------------
	
	\section*{Education}
	\vspace{-.4\textfloatsep}
	\renewcommand{\arraystretch}{1.25}
	
	\begin{tabularx}{.99\textwidth}{>{\setlength\hsize{.005\hsize}\raggedright}X>{\setlength\hsize{.07\hsize}\raggedright}X@{} >{\setlength\hsize{.875\hsize}\raggedright}X>{\setlength\hsize{.05\hsize}\raggedleft}X}
		\multicolumn{3}{l}{\textbf{University of Michigan,} Ann Arbor, MI.} & \tabularnewline
		& Ph.D. & Statistics & 2021 \tabularnewline 
		& & {\small \textit{Design and analytic considerations for sequential, multiple\hyp{}assignment randomized trials with continuous longitudinal outcomes.} 
			\small Advised by Daniel Almirall, Ph.D.} 
		\href{https://dx.doi.org/10.7302/2671}{\small \texttt{doi.org/n4r2}}\tabularnewline
		& M.A. & Statistics & 2018 \tabularnewline
		& M.S. & Biostatistics & 2015 \tabularnewline[.5em]
		\multicolumn{3}{l}{\textbf{University of Notre Dame,} Notre Dame, IN.} & \tabularnewline
		& B.S. & Mathematics (with Life Science), \textit{cum laude} & 2013 
	\end{tabularx}
	
	
	% \renewcommand{\arraystretch}{1.25}
	% \begin{tabularx}{.99\textwidth}{p{16mm}p{9mm}X}
		% 	2021 & Ph.D. & Statistics. University of Michigan. \\
		% 	&  & 
		% 	\hspace{3mm}\begin{minipage}{.99\linewidth}
			% 		\begin{flushleft}
				% 		{\small Advised by Daniel Almirall, Ph.D.} \\
				% 		\small ``Design and Analytic Considerations for Sequential, Multiple\hyp{}Assignment Randomized Trials with Continuous Longitudinal Outcomes.'' \href{https://dx.doi.org/10.7302/2671}{DOI: \texttt{n4r2}}
				% 		\end{flushleft}
			% 	\end{minipage} \vspace{\itemsep} \\
		% 	2018 & M.A. & Statistics. University of Michigan. \\[\itemsep]
		% 	2015 & M.S. & Biostatistics. University of Michigan. \\[\itemsep]
		% 	2013 & B.S. & Mathematics (with Life Science), \textit{cum laude}. University of Notre Dame.
		% \end{tabularx}
	% \renewcommand{\arraystretch}{1}
	
	\section*{Faculty \& Research Positions}
	\vspace{-.4\textfloatsep} 
	
	% \renewcommand{\arraystretch}{1}
	
	\begin{tabularx}{.99\textwidth}{>{\setlength\hsize{.005\hsize}\raggedright}X >{\setlength\hsize{.80\hsize}\raggedright}X@{}>{\setlength\hsize{.17\hsize}\raggedleft}X}
		\multicolumn{2}{l}{\textbf{University of Pennsylvania,} Philadelphia, PA.} & \tabularnewline
		& Senior Scholar, Center for Clinical Epidemiology and Biostatistics & 2025 - Present \tabularnewline
		& Assistant Professor of Biostatistics & 2023 - Present \tabularnewline[.5em]
		\multicolumn{2}{l}{\textbf{Johns Hopkins Bloomberg School of Public Health,} Baltimore, MD.} & \tabularnewline
		& Postdoctoral Fellow, Department of Health Policy and Management \\ {\small Advised by Elizabeth A. Stuart, Ph.D. and Emma E. (Beth) McGinty, Ph.D.} & 2021-2023 
	\end{tabularx}
	
	
	
	% \begin{tabularx}{.99\textwidth}{p{16mm}X}
		% 	\multicolumn{2}{l}{University of Pennsylvania Perelman School of Medicine} \\[.5em]
		%        \small 2025- & \small Senior Scholar, Center for Epidemiology and Biostatistics \\
		% 	\small 2023- & \small Assistant Professor of Biostatistics \\[.5em]
		
		%        \multicolumn{2}{l}{Johns Hopkins Bloomberg School of Public Health} \\
		% 	2021-2023 &
		% 	\hspace{3mm}\begin{minipage}{\linewidth}
			% 		\begin{flushleft}
				% 			{\small Department of Health Policy and Management} \\[-0.5mm]
				% 			{\small Advised by Elizabeth A. Stuart, Ph.D. and Emma E. (Beth) McGinty, Ph.D.}
				% 		\end{flushleft} 
			% 	\end{minipage}
		% \end{tabularx}
	% \renewcommand{\arraystretch}{1}
	
	\section*{Awards}
	\begin{etaremune}
		\item Chair's Junior Faculty Spotlight Award. Department of Biostatistics, Epidemiology, and Informatics, University of Pennsylvania Perelman School of Medicine, 2026.
		\item Excellence in Teaching. Johns Hopkins Bloomberg School of Public Health, AY 2022-23, First Term.
		\item Junior Researcher Travel Grant, American Causal Inference Conference, 2022. Supported by the National Science Foundation, awarded to Duke University.
		\item Brenda Gunderson Graduate Student Instructor Award. Department of Statistics, University of Michigan, 2021. (Inaugural recipient)
		\item Outstanding Graduate Student Instructor. Rackham Graduate School, University of Michigan, 2021. (University-wide)
		\item Outstanding Graduate Student Instructor. Department of Statistics, University of Michigan, 2020.
		\item Best Departmental Poster, Statistics. Michigan Student Symposium for Interdisciplinary Statistical Sciences, 2017.
		\item Best Departmental Poster, Biostatistics. Michigan Student Symposium for Interdisciplinary Statistical Sciences, 2015.
	\end{etaremune}
	
	\section*{Publications}
	
	\setlength\bibitemsep{\itemsep}
	
	{\small%
		%\underline{Underline} 
		\GetAnnotSymbol{student} denotes trainee with significant mentoring.}
	
	\subsection*{Peer-Reviewed Articles}
    	\begin{refsection}[peerreviewed]
    		\nocite{*}
    		\printbibliography[heading=none,type=article]
    	\end{refsection}	
	
	\subsection*{Peer-Reviewed Book Chapters}
    	\begin{refsection}
    		\nocite{*}
    		\printbibliography[heading=none,type=incollection]
    	\end{refsection}

    \subsection*{Abstracts}
        \begin{refsection}[abstracts]
            \nocite{*}
            \printbibliography[heading=none,type=inproceedings]
        \end{refsection}
		
	\subsection*{Under Review}
    	\begin{refsection}[underreview]
    		\nocite{*}
    		\printbibliography[heading=none]
    	\end{refsection}
	
	\subsection*{In Preparation}
    	\begin{etaremune}
    		\item \textbf{Seewald NJ}, *Cohen M, McGinty EE, and Stuart EA. ``Ready to Roll? Whether and when to aggregate multilevel data in difference-in-differences.'' 
    		
    		\item \textbf{Seewald NJ}, *Jasper R, and Almirall D. ``Sample size and timepoint tradeoffs for comparing dynamic treatment regimens in a longitudinal SMART.''
    	\end{etaremune}
	
	\subsection*{Research Publications (not peer reviewed)}
        \begin{refsection}[preprints]
            \nocite{*}
            \printbibliography[heading=none]
        \end{refsection}
	
	% \begin{plnumgroup}
		%     \nocite{*}
		%     \printbibliography[heading=none,type=online]
		% \end{plnumgroup}
	
	% \begin{etaremune}
		%           \item \underline{Moin EE}, \textbf{Seewald NJ}, Halpern SD ``Development and Validation of a Simple Model to Predict Patient Height.'' \href{https://www.medrxiv.org/content/10.1101/2025.03.12.25323846}{medRxiv: \texttt{2025.03.12.25323846}}
		%           \item \textbf{Seewald NJ}, McGinty EE, Tormohlen K, Schmid I, Stuart EA ``Shared control individuals in state-level health policy evaluation.'' \href{https://arxiv.org/abs/2311.18093}{arXiv: \texttt{2311.18093}} [stat.ME]
		% 	\item \textbf{Seewald NJ} (2023) ``Adaptive Interventions for a Dynamic and Responsive Public Health Approach,'' \textit{American Journal of Public Health}, 113:1, 37-39. \href{https://doi.org/jqkz}{DOI: \texttt{jqkz}}.
		%     \item \textbf{Seewald NJ}, Sun J, Liao P (2016), ``MRT-SS Calculator: An R Shiny Application for Sample Size Calculation in Micro-Randomized Trials.'' \href{http://arxiv.org/abs/1609.00695}{arXiv: \texttt{1609.00695}} [stat.ME]
		% \end{etaremune}
	
	\subsection*{Alternative Media}
	
	% \begin{refsection}[altMedia]
		%     \nocite{*}
		%     \printbibliography[heading=none]
		% \end{refsection}
	
	\begin{etaremune}
		\item Morabia A (host), Chakraborty B, \textbf{Seewald NJ} (2023) ``SMART and MRT - Emerging methods in public health,'' \textit{AJPH Podcast}, American Public Health Association. September 13. Available at \url{https://youtu.be/TER4kRJpCh0}.
	\end{etaremune}
	
	
	\section*{Teaching Experience}
	
	\subsection*{University of Pennsylvania Perelman School of Medicine}
	
	\subsubsection*{Course Director}
	\begin{etaremune}
        \item BSTA 7000: Seminal Ideas and Controversies in Statistics \hfill Fall 2026 \\
        Joint with Nandita Mitra, Ph.D.
		\item BSTA 6100: Biostatistical Methods for Epidemiology \hfill Fall 2024, 2025
	\end{etaremune}
	
	\subsubsection*{Guest Lecturer}
	\begin{etaremune}
		\item Penn Causal Summer Institute \hfill 2024 - Present \\
		``Quasi-Experimental Designs.'' \\
		Co-Directors: Nandita Mitra, Ph.D.; Eric Tchetgen-Tchetgen, Ph.D.
		\item EPID 7020: Advanced Topics in Epidemiologic Research \hfill Spring 2025, 2026 \\
		``Quasi-Experimental Designs.'' \\
		Course Director: Ellen C. Caniglia, Ph.D.
	\end{etaremune}
	
	\subsection*{Johns Hopkins Bloomberg School of Public Health}
	
	\subsubsection*{Co-Instructor}
	\begin{etaremune}
		\item Seminar on Statistical Methods for Mental Health \hfill Term 1, AY 2022-3 \\
		``Promises and Pitfalls of Prediction Models in Mental Health Research'' \\
		Co-taught with Trang Q. Nguyen, Ph.D.
	\end{etaremune}
	
	\subsubsection*{Guest Lecturer}
	\begin{etaremune}
		\item Seminar on Statistical Methods for Mental Health \hfill Term 3, AY 2021-2 \\
		``Difference in Differences with Variable Treatment Timing.'' \\
		Instructor: Elizabeth A. Stuart, Ph.D.
	\end{etaremune}
	
	\subsection*{University of Michigan}
	
	\subsubsection*{Graduate Student Mentor}
	\begin{enumerate}
		\item \textit{STATS 250: Introduction to Statistics and Data Analysis} \hfill AY 2019-20, AY 2020-1 \\
		Instructor: Brenda Gunderson, Ph.D.; Jack Miller, Ph.D.
		% 			\vspace{-\topsep}
		% 			\begin{itemize}
			% 			    \itemsep-1.4em 
			% 			    \item Mentored seven first-time graduate student instructors through their first teaching experiences.
			% 			\end{itemize}
	\end{enumerate}
	
	\subsubsection*{Graduate Student Instructor}
	\begin{etaremune}
		\item \textit{STATS 250: Introduction to Statistics and Data Analysis} \hfill Spring 2018, AY 2019-20, 20-1 \\
		Instructor: Brenda Gunderson, Ph.D.; Jack Miller, Ph.D.
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item Large, non-calculus-based, cross-disciplinary introductory statistics course.
			%			\end{itemize}
		
		\item \textit{STATS 415: Data Mining} \hfill Winter 2018 \\
		Instructor: Liza Levina, Ph.D. 
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item Upper-level undergraduate introductory machine learning course using \textit{An Introduction to Statistical Learning} (James, Witten, Hastie, Tibshirani).
			%			\end{itemize}
		
		\item \textit{STATS 500: Statistical Learning I: Regression} \hfill Fall 2017 \\
		Instructor: Brian Thelan, Ph.D.
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item First graduate-level regression course for Applied Statistics masters students.
			%			\end{itemize}
	\end{etaremune}
	
	\subsubsection*{Instructional Assistant}
	\begin{enumerate}
		\item \textit{STATS 250: Introduction to Statistics and Data Analysis} \hfill Summer 2020 \\
		Supervisor: Jack Miller, Ph.D.
		%		\vspace{-\topsep}
		%		\begin{itemize}
			%			\item Aided in curriculum development for transition to simulation-based inference in large, introductory undergraduate statistics course.
			%			\item Redesigned lab experiences to involve in-depth data analysis projects using R code in R Markdown.
			%		\end{itemize}
	\end{enumerate}
	
	\subsection*{Inter-university Consortium for Political and Social Research (ICPSR)}
	\subsubsection*{Teaching Assistant, Summer Program in Quantitative Methods of Social Research}
	\begin{etaremune}
		\item \textit{Multilevel Models I: Introduction and Application} \hfill Summer 2018, Summer 2019\\
		Instructor: Mark Manning, Ph.D.
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item Four-week project-based course for political and social scientists interested in mixed modeling.
			%			\end{itemize}
		
		\item \textit{Introduction to the R Statistical Computing Environment} \hfill Summer 2018, Summer 2019 \\
		Instructor: John Fox, Ph.D.
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item Two-week lecture series on graphics, data management, modeling, etc., in R.\\
			%			\end{itemize}
	\end{etaremune}
	
	\subsection*{University of Notre Dame}
	\subsubsection*{Undergraduate Teaching Assistant}
	\begin{enumerate}
		\item \textit{BIOS 40411: Biostatistics} \hfill Spring 2012 \\
		Instructor: Gary Lamberti, Ph.D.
		% \vspace{-\topsep}
		% \begin{itemize}
			% \itemsep-1.4em
			% \item Advanced, non-calculus-based introduction to statistics for undergraduate life science majors. \\
			% \item Co-taught with a graduate TA weekly labs, held office hours, graded homework and exams.
			% \end{itemize}
	\end{enumerate}
	
	\section*{Mentorship Experience}
	
	\subsection*{Academic Advisees}

    \subsubsection*{Ph.D. Students}
	\initRevTable{phdStudents}
	\begin{tabularx}{\textwidth}{>{\rowcount}lXXr}
		\autoRevNum & Ryan Jasper & Ph.D. Biostatistics & 2025 - Present \\
		\autoRevNum & Meghan Gerety & Ph.D. Biostatistics & 2025 - Present \\
		\autoRevNum & Melissa Martin \newline {\footnotesize Joint with Ian Barnett, Ph.D.} & Ph.D. Biostatistics \newline {\footnotesize Senior Scientist (Biostatistics), Merck} & 2024 - 2026
	\end{tabularx}

    \subsubsection*{Masters Students}
    \initRevTable{msStudents}
    \begin{tabularx}{\textwidth}{>{\rowcount}lXXr}
        \autoRevNum & Daniel Goldblatt \newline {\footnotesize Joint with Jarcy Zee, Ph.D.} & M.S. Biostatistics & 2026 - Present \\
        \autoRevNum & Derek Lamb \newline {\footnotesize Joint with Nandita Mitra, Ph.D.} & M.S. Biostatistics & 2026 - Present \\
        \autoRevNum & Hao Tong, M.D. & M.S. Clinical Epidemiology & 2025 - Present \\
        \autoRevNum & Ryan Jasper \newline {\footnotesize Joint with Jing Huang, Ph.D.} & M.S. Biostatistics & 2025 - 2026 \\
		\autoRevNum & Luke Stewart \newline {\footnotesize Joint with Jinbo Chen, Ph.D.} &  M.S. Biostatistics & 2025 - 2026 \\
        \autoRevNum & Emily Moin, M.D., M.B.E. & M.S. Clinical Epidemiology & 2023 - 2025 \\
        \autoRevNum & Eleanor Taranto, M.D. & M.S. Clinical Epidemiology & 2023 - 2025
    \end{tabularx}	
	
	\subsection*{Ph.D. Committee Membership}
	\initRevTable{Committees}
	\begin{tabularx}{\linewidth}{>{\rowcount}lXXr}
	  \autoRevNum & Jimmy Kelliher \newline {\footnotesize Committee Chair} & Ph.D. Biostatistics & 2024 - Present
	\end{tabularx}
	
	\subsection*{Biostatistics Ph.D. Lab Rotation Advisees}
	\begin{enumerate}
		\item Ryan Jasper \hfill Fall 2024, Summer 2025
		\item Meghan Gerety \hfill Spring 2024 - Spring 2025 \\
		\begin{minipage}{\linewidth}
			\begin{flushleft}
				{\small Co-advised with Rebecca Hubbard, Ph.D., in Spring 2024}
			\end{flushleft}
		\end{minipage}
	\end{enumerate}

    \subsection*{Undergraduate Students}
    \begin{etaremune}
        \item Mason Cohen \hfill Summer - Fall 2024
    \end{etaremune}
	
	\section*{Consulting Experience}
	\subsubsection*{Statistical Consultant}
	\begin{enumerate}
		\item \textit{Committee for Children}, Seattle, WA \hfill May - December 2021
	\end{enumerate}
	
	\subsubsection*{Graduate Student Consultant}
	\begin{enumerate}
		\item \textit{Consulting for Statistics, Computing, and Analytics Research (CSCAR)} \hfill Fall 2018 \\
		University of Michigan
		%			\vspace{-\topsep}
		%			\begin{itemize}
			%			    \itemsep-1.4em
			%			    \item Held one-hour walk-in consulting sessions on study design, sample size, data management, data visualization, and statistical analysis.
			%			\end{itemize}
	\end{enumerate}
	
	
	\section*{Presentations and Posters}
	
	\subsection*{Presentations at Scientific Meetings}
	
	\begin{etaremune}
        \item Statistical considerations for interventional oncology-specific trial designs
        \begin{conflist}
            \item Society of Interventional Oncology Annual Scientific Meeting. Savannah, GA. February 2026. \textit{Invited}.
        \end{conflist}
		\item Ecological regression in health policy evaluation: A guilt-free dessert
		\begin{conflist}
			\item Society for Epidemiologic Research Annual Meeting. Boston, MA. June 2025. \textit{Invited.}
		\end{conflist}
		\item How to ask the right question: Choosing estimands for health policy research
		\begin{conflist}
			\item International Conference on Computational and Methodological Statistics. London, United Kingdom. December 2025. \textit{Invited.}
			\item ENAR Spring Meeting. New Orleans, LA. March 2025. \textit{Invited.}
		\end{conflist}
		\item Crafting relevant research: Informative and translational statistics for public policy
		\begin{conflist}
			\item Joint Statistical Meetings. Portland, OR. August 2024. \textit{Invited Panelist}.
		\end{conflist}
		\item Target trial emulation for evaluating mental health policy
		\begin{conflist}
			\item Thomas R. Ten Have Symposium on Statistics in Mental Health. New York, NY. June 2024. \textit{Invited.}
		\end{conflist}
		\item Practical guidance on whether and when to aggregate individual-level data for causal health policy evaluation
		\begin{conflist}
			\item American Causal Inference Conference. Seattle, WA. May 2024.
		\end{conflist}
		\item Target trials in policy evaluation: A case study in medical cannabis laws
		\begin{conflist}
			\item Society for Epidemiologic Research Annual Meeting. Portland, OR. June 2023. \textit{Invited.}
		\end{conflist}
		\item Handling correlation in stacked difference-in-differences estimates with application to medical cannabis policy
		\begin{conflist}
			\item International Conference on Computational and Methodological Statistics. Berlin, Germany. December 2023. \textit{Invited.}
			\item American Causal Inference Conference. Austin, TX. May 2023.
			\item Joint Statistical Meetings. Washington, DC. August 2022.
		\end{conflist}
		\item Ready to Roll? Practical guidance on whether and when to aggregate data in health policy evaluation
		\begin{conflist}
			\item International Conference on Health Policy Statistics. Scottsdale, AZ. January 2023.
		\end{conflist}\item Policy evaluation in the real world: Experiences with translating cutting-edge methods for an applied audience
		\begin{conflist}
			\item American Causal Inference Conference. Berkeley, CA. May 2022.
		\end{conflist}
		\item Budgeting SMART: Sample size and repeated measures with a cost  constraint in a longitudinal sequential, multiple-assignment randomized trial.
		\begin{conflist}
			\item  ENAR Spring Meeting. Virtual. March 2021.
		\end{conflist}
		\item Sample size and timepoint tradeoffs for comparing dynamic treatment regimens in a longitudinal SMART.
		\begin{conflist}
			\item Joint Statistical Meetings. Virtual. August 2020.
		\end{conflist}
		\item Design considerations for comparing dynamic treatment regimens  in a longitudinal SMART.
		\begin{conflist}
			\item ENAR Spring Meeting. Virtual. March 2020.
		\end{conflist}
		\item Sample size considerations for comparing dynamic treatment regimes in a SMART with a longitudinal outcome.
		\begin{conflist}
			\item International Conference on Computational and Methodological Statistics. London, United Kingdom. December 2019.
			\item Joint Statistical Meetings. Denver, CO. July 2019.
		\end{conflist}
		\item Sample size considerations for comparing dynamic treatment regimens in a sequentially-randomized trial with a continuous longitudinal outcome.
		\begin{conflist}
			\item Society for Clinical Trials Annual Meeting. New Orleans, LA.  May 2019.
		\end{conflist}
		\item Sample size considerations for comparing dynamic treatment regimens in a sequential multiple-assignment randomized trial with a continuous longitudinal outcome.
		\begin{conflist}
			\item Joint Statistical Meetings. Vancouver, BC, Canada. July 2018.
			\item ENAR Spring Meeting. Atlanta, GA.  March 2018.
		\end{conflist}		
	\end{etaremune}
	
	\subsection*{Invited Colloquia}
	\begin{enumerate}
		\item Design thinking for policy evaluation: The policy trial emulation framework
		\begin{conflist}
			\item Department of Statistics, North Carolina State University. Raleigh, NC. April 2025.
		\end{conflist}
		\item Target trial emulation for evaluating health policy
		\begin{conflist}
			\item Philadelphia Chapter, American Statistical Association. Virtual. March 2024.
		\end{conflist}
		\item Using individual-level data in difference-in-differences for health policy evaluation.
		\begin{conflist}
			\item Department of Biostatistics and Data Science, Wake Forest University School of Medicine. Virtual. March 2023.
			\item Department of Biostatistics, Epidemiology and Informatics, University of Pennsylvania Perelman Medical School. Virtual. March 2023.
			\item Department of Public Health Sciences, Henry Ford Health. Virtual. February 2023.
			\item Division of Biostatistics, Northwestern University Feinberg School of Medicine. Chicago, IL. January 2023.
			\item Departments of Population Health Sciences and Biostatistics and Medical Informatics, University of Wisconsin-Madison. Madison, WI. January 2023.
			\item Department of Biostatistics, University of Colorado Anschutz Medical Campus. Virtual. January 2023.
			\item Johns Hopkins ALACRITY Center Research in Progress Meeting. Virtual. November 2022.
		\end{conflist}
		\item A brief introduction to multi-stage trials for developing dynamic treatment regimens.
		\begin{conflist}
			\item MNeuroNetwork Lab Seminar, University of Michigan. Virtual.  March 2022. 
			\item Working Group on Clinical Research, University of Rochester Medical Center. Virtual. October 2021.
		\end{conflist}
		\item Design, analysis, and sizing of sequential multiple assignment randomized trials with binary outcomes.
		\begin{conflist}
			\item Graduate Student Statistical Topics Seminar Series, Department of Statistics, University of Michigan. September 2015.
		\end{conflist}
	\end{enumerate}
	
	\subsection*{Posters}
	\begin{enumerate}
		\item ``Ready to roll? Practical guidance on whether and when to aggregate data in health policy evaluation''
		\begin{conflist}
			\item AcademyHealth Annual Research Meeting. Washington, DC. June 2022.
		\end{conflist}
		\item Sample size considerations for comparing dynamic treatment regimens in  a sequential multiple-assignment randomized trial with a continuous longitudinal outcome.
		\begin{conflist}
			\item Michigan Student Symposium for Interdisciplinary Statistical Sciences. Ann Arbor, MI. March 2019.
			\item Thomas R. Ten Have Symposium on Statistics in Mental Health. Chicago, IL. June 2018.
		\end{conflist}
		\item Sample size considerations for the analysis of continuous repeated-measures outcomes in sequential multiple-assignment randomized trials.
		\begin{conflist}
			\item Michigan Student Symposium for Interdisciplinary Statistical Sciences. Ann Arbor, MI. March 2017.
		\end{conflist}
		\item A SMART web-based sample size calculator.
		\begin{conflist}
			\item Michigan Student Symposium for Interdisciplinary Statistical Sciences. Ann Arbor, MI. March 2015.
			\item IMPACT Symposium III: Advances in Clinical Trial Statistics: Multiplicity Adjustment and SMARTs. Cary, NC. November 2014.
		\end{conflist}
	\end{enumerate}
	
	\section*{Software}
	
	\begin{enumerate}
		\item SMARTsize: A sample size calculator for sequential, multiple-assignment, randomized trials with binary or continuous outcomes in which the primary aim is to compare two embedded dynamic treatment regimes. \\ \url{https://nseewald1.shinyapps.io/SMARTsize/}
		\item MRT-SS Calculator: A sample size calculator for micro-randomized trials in which the primary aim is to detect the proximal effect of providing an intervention.\\ \url{https://pengliao.shinyapps.io/mrt-calculator/}
	\end{enumerate}
	
	\section*{Professional Service}
	
	\subsection*{Public Training Materials}
	
	\begin{enumerate}
		\item ``Target Trial Emulation for Evaluating Health Policy.'' \\
		Johns Hopkins ALACRITY Center for Health and Longetivity in Mental Illness \\
		\url{https://www.youtube.com/watch?v=DAXfp8X9ba8}
		
		\item ``Building Effective Adaptive Interventions in Mental Health Services Research.'' \\
		Johns Hopkins ALACRITY Center for Health and Longevity in Mental Illness \\
		\url{https://youtube.com/playlist?list=PLj4-nn-p-zv1o-cR9jiITTRfgkwmyqjoD}
	\end{enumerate}
	
	\subsection*{Workshop Design and Facilitation}
	
	\begin{enumerate}
        \item Society of Interventional Oncology Clinical Trials Methodology Workshop
        \begin{conflist}
            \item Steering Committee member and biostatistics faculty. Philadelphia, PA. May 19-23, 2026.
        \end{conflist}
    
		\item ``SMART Designs for Developing Adaptive Interventions''
		\begin{conflist}
			\item Statistical Horizons, Virtual. July 8-11, 2025.
			\item Statistical Horizons, Virtual. August 19-23, 2024.
		\end{conflist}
		
		\item ``Embedding Qualitative and Quantitative Methods for Policy Evaluation''
		\begin{conflist}
			\item Invited pre-conference workshop: 15th International Conference on Health Policy Statistics, San Diego, CA. January 6, 2024. \textit{Co-designer and facilitator with Beth McGinty, Ph.D. and Eli Ben-Michael, Ph.D.}
		\end{conflist}
		
		\item ``Introduction to Adaptive Interventions and Sequential Multiple Assignment Randomized Trials''
		\begin{conflist}[noitemsep]
			\item Invited pre-conference workshop: Society of Behavioral Medicine Annual Meeting, Baltimore, MD. April 6, 2022. \textit{Co-designer and facilitator with Ahnalee Brincks, Ph.D., and Shawna N. Smith, Ph.D.}
		\end{conflist}
		
		\item ``Building Just-in-Time Adaptive Interventions.'' 
		\begin{conflist}[noitemsep]
			\item Center for Dissemination and Implementation Science Summer Institute, University of Illinois Chicago. Virtual. October 25-26, 2021. \textit{Small group expert facilitator}.
		\end{conflist}
		
		\item ``Using SMART Design in Responsive Survey Design.'' 
		\begin{conflist}
			\item Responsive Survey Design: A Research Education Program. 
			Institute for Social Research, University of Michigan. June 19, 2019. \textit{Co-designer and facilitator with Ahnalee Brincks, Ph.D.}
		\end{conflist}
		
		\item ``Getting SMART about Adaptive Interventions in Education.''
		\begin{conflist}
			\item Institute of Education Sciences-funded training institute at the University of Michigan. March 11-14, 2019. \textit{Co-designer and facilitator with d$^3$lab} \href{https://d3c.isr.umich.edu/}{\texttt{d3lab.isr.umich.edu}}.
		\end{conflist}
		
		\item ``Getting SMART: Experimental Design and Analysis Methods for Developing Adaptive Interventions''
		\begin{conflist}
			\item University of California, San Francisco. May 19, 2015. \textit{Co-facilitator with Inbal Nahum-Shani, Ph.D.}
		\end{conflist}
		
		\item ``Experimental Design and Analysis Methods for Developing Adaptive Interventions: Getting SMART''
		\begin{conflist}
			\item Methodology Center Summer Institute. Ann Arbor, MI. June 19-20, 2014. \textit{Small group facilitator}.
		\end{conflist}
	\end{enumerate}
	
	\subsection*{Peer Review and Editorial Activities}
	\subsubsection*{Editorial}
	\begin{etaremune}
        \item Associate Editor, American Journal of Epidemiology. 2026 - Present.
		\item Guest Academic Editor, PLOS Medicine. 2025. 
	\end{etaremune}
	
	\subsubsection*{Ad hoc Reviewer, Academic Journals}
    \begin{minipage}[l]{1.25em}
        \hfill
    \end{minipage}    
    \begin{minipage}[r]{\linewidth - 1.25em}
        \noindent
        \begin{itemize*}[before=\kern-0.3em, itemjoin={{~\textperiodcentered~}}, afterlabel=\unskip{{~}}]
		\item[] American Journal of Epidemiology
		\item[] American Journal of Public Health
		\item[] Annals of Applied Statistics
		\item[] Behavior Research Methods
        \item[] Biometrics
		\item[] BMJ
		\item[] Clinical Trials
		\item[] Contemporary Clinical Trials
		\item[] Data Science in Science
		\item[] Frontiers in Digital Health
		\item[] Health Services Research
		\item[] JAMA Health Forum
		\item[] JAMA Network Open
		\item[] JAMA Pediatrics
		\item[] Journal of the American Statistical Association (Theory and Methods)
        \item[] Journal of Causal Inference
		\item[] Pharmacoepidemiology and Drug Safety
        \item[] PLOS Medicine
		\item[] Psychological Methods
		\item[] R Journal
		\item[] Statistics and Public Policy
		\item[] Statistics in Medicine
		\item[] Value in Health
	\end{itemize*}
    \end{minipage}

    \subsubsection*{Grant Review}
    \begin{etaremune}
        \item Scientist Reviewer (methodologist), PCORI Merit Review Panel. Patient-Centered Outcomes Research Institute (PCORI). 2026.
        \item Panel Member, Pharmacotherapy of Substance Use Disorder (PSUD). National Institutes of Health (NIH). 2026.
        \item Subject Matter Expert reviewer of Draft Final Report. Patient-Centered Outcomes Research Institute (PCORI). 2025.
    \end{etaremune}
	
	\subsection*{Departmental Service}
	\begin{etaremune}
        \item Member, Biostatistics Faculty Recruitment Committee. Department of Biostatistics, Epidemiology, and Informatics, University of Pennsylvania. 2025 - Present.
		\item Member, Biostatistics Seminar Committee. Department of Biostatistics, Epidemiology, and Informatics, University of Pennsylvania. 2025 - Present.
		\item Member, Epidemiology Qualifying Exam Committee. Graduate Group in Epidemiology and Biostatistics, University of Pennsylvania. 2025 - Present.
		\item Co-Chair, Junior Faculty Committee. Department of Biostatistics, Epidemiology, and Informatics, University of Pennsylvania. 2023 - Present.
		\item Member, Biostatistics Student Recruitment Committee. Graduate Group in Epidemiology and Biostatistics, University of Pennsylvania. 2023 - Present.
		\item Student Representative, Faculty Diversity, Equity, and Inclusion Committee. Department of Statistics, University of Michigan. AY 2020-2021.
		\item Co-Lead, Graduate Student Instructor Foundations Colloquium. Department of Statistics, University of Michigan. AY 2020-2021.
		\item Steering Committee Member, Graduate Student Diversity, Equity, and Inclusion Working Group. Department of Statistics, University of Michigan. 2020-2021.
	\end{etaremune}
	
	\subsection*{Professional Memberships}
	\begin{enumerate}[noitemsep, leftmargin = 1.25em]
		\item[] American Statistical Association (2015 - present)
		\item[] International Biometrics Society, Eastern North American Region (ENAR) (2015 - present)
		\item[] Society for Causal Inference (2023 - present)
		\item[] Society for Clinical Trials (2015 - 2021)
		\item[] Society of Industrial and Applied Mathematics (SIAM) (2020-2021)
		\item[] Society of Behavioral Medicine (2022 - 2023)
	\end{enumerate}
	
	\subsection*{Organizational Activities}
	\begin{etaremune}
        \item Member, Program Committee, 2027 American Causal Inference Conference. Society for Causal Inference. 2026-2027.
        \item Member, Outreach Committee, 16th International Conference on Health Policy Statistics. 2026.
        \item Abstract Concurrent Session Organizer, 2026 American Causal Inference Conference. Society for Causal Inference. 2025-2026.
		\item Member, Program Committee, 18th Annual Conference on Statistical Issues in Clinical Trials. University of Pennsylvania. 2025-2026.
		\item Member, Program Committee, 2026 American Causal Inference Conference. Society for Causal Inference. 2025-2026.
		\item Session Chair, ENAR Spring Meeting. March 2025.
		\item Chair, Diversity Equity and Inclusion Committee. Society for Causal Inference. 2025.
		\item Member, Program Committee, 17th Annual Conference on Statistical Issues in Clinical Trials. University of Pennsylvania. 2024-2025.
		\item Member, Outreach Committee, 15th International Conference on Health Policy Statistics. 2024.
		\item Session Chair, 14th International Conference on Health Policy Statistics. January 2023.
		\item Session Chair, Joint Statistical Meetings. August 2022.
	\end{etaremune}
	
	\subsection*{Professional Development}
	
	\renewcommand{\arraystretch}{1.25}
	\begin{etaremune}
		\item U-M Graduate Teacher Certificate. Rackham Graduate School and Center for Research on Learning and Teaching, University of Michigan. Completed April 2021. \href{https://crlt.umich.edu/um.gtc/description}{crlt.umich.edu/um.gtc} 
		\item Inclusive STEM Teaching Project. Completed December 2020. \href{https://www.inclusivestemteaching.org/}{inclusivestemteaching.org}
		\item Preparing to Teach: Workshop to prepare graduate students for a role as undergraduate faculty responsible for teaching statistics and data science. May 2020. \href{https://preparingtoteach.org/}{preparingtoteach.org}
		\item Professional Development Diversity, Equity, and Inclusion Certificate. Rackham Graduate School, University of Michigan. Completed April 2020. \href{https://rackham.umich.edu/professional-development/dei-certificate/}{rackham.umich.edu}
	\end{etaremune}

    \section*{Research Grants}
	
    \subsection*{Funded}
	\begin{enumerate}
		\item Impact of U.S. State Recreational Cannabis Laws on Opioid Prescribing for Chronic Non-Cancer Pain and Statistical Methods for Informative Policy Adoption
		\begin{description}[noitemsep, topsep=0pt, labelindent=1em]
			\item[Role] Principal Investigator
			\item[Funding Source] Thomas B. McCabe and Jeannette E. Laws McCabe Fund \\ \url{https://www.med.upenn.edu/evdresearch/mccabe-fund.html}
			\item[Funding Level] Pilot Award, 7/1/24 - 12/31/25: \$36,347
			%		\item[Main Objective]  To use mixed-methods to characterize the implementation and evaluate the effect of state medical cannabis laws on receipt of guideline-discordant opioid and guideline-concordant non-opioid treatment among patients diagnosed with chronic non-cancer pain.
		\end{description}
	\end{enumerate}
	
	% \begin{enumerate}
		% 	\item State Medical Cannabis Laws, Chronic Pain, and Opioids: A Mixed-Methods Approach
		% 	\begin{description}[noitemsep, topsep=0pt, labelindent=1em]
			% 		\item[Principal Investigator] Beth McGinty (1R01DA049789)
			% 		\item[Funding Source] National Institutes of Health, National Institute on Drug Abuse
			% %		\item[Funding Level] 7/1/20-6/30/24: \$1,751,547 (direct)
			% %		\item[Main Objective]  To use mixed-methods to characterize the implementation and evaluate the effect of state medical cannabis laws on receipt of guideline-discordant opioid and guideline-concordant non-opioid treatment among patients diagnosed with chronic non-cancer pain.		
			% 		\item[Role] Investigator 
			% 	\end{description}
		% \end{enumerate}
	% \subsection*{Under Review}
	% 	\begin{enumerate}
		% 		\item Center to Accelerate Translation of Interventions to Decrease Premature Mortality in SMI
		% 		\begin{description}[noitemsep, topsep=0pt, labelindent=1em]
			% 			\item[Principal Investigator] Gail L. Daumit
			% 			\item[Funding Source] National Institutes of Health, National Institute of Mental Health
			% 			\item[Role] Project Leader, Methods Core (PI: E.A. Stuart) 
			% 			\item[Submission Target] May 2023
			% 		\end{description}
		% 	\end{enumerate}
	% %\subsection*{Pending Submission}
	% \subsection*{Not Funded}
	% \begin{enumerate}
		% 	\item iHear: Digital Hearing Healthcare Delivery Platform for OTC Hearing Aids
		% 	\begin{description}[noitemsep, topsep=0pt, labelindent=1em]
			% 		\item[Principal Investigators] Sumitrajit Dhar and Larry Humes
			% 		\item[Funding Source] National Institutes of Health, National Institute on Deafness and Other Communication Disorders
			% 		\item[Role] Consultant, statistics and trial design
			% 	\end{description}
		% \end{enumerate}	
	


    
	%\section*{References}
	%\begin{center}
	%	\small
	%\begin{minipage}[t]{.3\textwidth}
	%	\textbf{Elizabeth A. Stuart, Ph.D.} \\
	%	Johns Hopkins Bloomberg School of Public Health \\
	%	624 N. Broadway \\
	%	Hampton House 839 \\
	%	Baltimore, MD 21205\\
	%	(410) 502-6222 \\
	%	\href{mailto:estuart@jhu.edu}{estuart@jhu.edu}
	%\end{minipage}
	%\hspace{.06\textwidth}
	%\begin{minipage}[t]{.3\textwidth}
	%	\textbf{Beth McGinty, Ph.D.} \\
	%	Weill Cornell Medicine \\
	%	402 E. 67th St \\
	%	LA-213 \\
	%	New York, NY 10065\\
	%	(646) 962-8038 \\
	%	\href{mailto:emm4010@med.cornell.edu}{emm4010@med.cornell.edu}
	%\end{minipage}
	%\hspace{.01\textwidth}
	%\begin{minipage}[t]{.3\textwidth}
	%	\textbf{Daniel Almirall, Ph.D.} \\
	%	University of Michigan \\
	%	426 Thompson Street \\
	%	2448 Institute for Social Research \\
	%	Ann Arbor, MI 48104 \\
	%	(734) 936-3077 \\
	%	\href{mailto:dalmiral@umich.edu}{dalmiral@umich.edu}
	%\end{minipage}
	%\end{center}
	
\end{document}